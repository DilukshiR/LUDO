...
let lastState = null;
let anim = null; // {startTime, duration, fromXY, toXY, p, k}
...
// We will render a "ring" path around the board as 52 evenly spaced cells.
// This is not the exact classic Ludo geometry, but it's consistent and easy.
// Later, you can replace these with exact Ludo cell coordinates.
...
function pieceXY(state, player, k){
  let pos = state.pieces?.[player]?.[k];

  // Allow object-based piece state from backend: { position, homeStraight, isHomeStraight }
  if (pos && typeof pos === "object"){
    if (typeof pos.position === "number") pos = pos.position;
  }

  if (pos === -1) return baseXY(player, k);
  if (pos >= 0 && pos <= 51) return ringXY(pos);

  // Home path support:
  // 1) If backend uses 52..57 mapping, use pos-52.
  if (pos >= 52 && pos <= 57) return homeXY(player, pos - 52);

  // 2) If backend uses -2 for home path, look for a separate home step.
  if (pos === -2){
    const hs = state.home?.[player]?.[k]
      ?? state.homeStraight?.[player]?.[k]
      ?? state.pieces?.[player]?.[k]?.homeStraight;
...
function onNewState(s){
  // detect piece moved, create simple animation
  if (lastState && s && lastState.pieces && s.pieces){
    let moved = null;
    for(let p=0;p<4;p++){
      for(let k=0;k<4;k++){
        if (lastState.pieces[p][k] !== s.pieces[p][k]){
          moved = { p, k };
          break;
        }
      }
  ...
  for(let p=0;p<4;p++){
    for(let k=0;k<4;k++){
      ...
      if (anim && anim.p === p && anim.k === k){
        ...
        pt = { x: lerp(anim.from.x, anim.to.x, easeOut(t)),
